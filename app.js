...
async function doLogin() {
  const ic = document.getElementById('ic-input').value.trim();
  ...
  if (!ic || ic.length !== 12) {
    errEl.textContent = 'Sila masukkan No. IC yang sah (12 digit).';
  ...
  try {
    ...
    if (!result.success) {
      errEl.textContent = result.message || 'No. IC tidak ditemui.';
      errEl.style.display = 'block';
      return;
    }
...
async function loadAllData() {
  try {
    const [reportsRes, laporanRes, courseRes, programRes] = await Promise.all([
...
const NAV_ITEMS = [
  { sep: 'Menu Utama' },
  { id: 'dashboard', icon: '📊', label: 'Papan Pemuka' },
...
function renderDashboard() {
  ...
  return `
    <div class="page-title">Papan Pemuka</div>
    ...
    <div class="stats-grid">
      ...
      <div class="stat-card"><div class="stat-label">Menunggu Ketua Program</div><div class="stat-value">${pendingKetua}</div></div>
    </div>
...
function openReportForm(id) {
    ...
    <div class="modal">
      ...
      <div class="section-block">
        ...
        <div class="form-grid mt-2">
          ...
          <div class="form-group"><label>Sesi Semasa</label><input id="f-sesi" value="${esc(existing?.Sesi)}" placeholder="cth: II 2025/2026"></div>
          <div class="form-group"><label>Sesi Lepas</label><input id="f-sesi-lepas" value="${esc(existing?.SesiLepas)}" placeholder="cth: I 2025/2026"></div>
        ...
        <div class="mt-2">
          ...
          <div class="repeat-header lecturer-row mt-1"><span>Kelas / Pensyarah</span><span></span></div>
  ...
  if (lecturers.length) lecturers.forEach(l => addLecturerRow(l)); else addLecturerRow();
...
function addLecturerRow(val) {
  ...
  const row = document.createElement('div');
  row.className = 'repeat-row lecturer-row';
  row.innerHTML = `
    <input type="text" class="lecturer-input" value="${esc(val || '')}" placeholder="cth: Kelas A — Dr. Ahmad bin Ali">
...
async function saveReportForm() {
  ...
  try {
    ...
    const lecturers = Array.from(document.querySelectorAll('.lecturer-input')).map(i => i.value).filter(Boolean);
    ...
    <div class="modal" style="max-width:820px;">
      ...
      <div class="section-block">
        ...
        <table style="font-size:13px;margin-top:8px;">
          ...
          <tr><td class="text-muted">Pensyarah</td><td>${lecturers.map(esc).join('<br>') || '—'}</td></tr>
        </table>
      </div>
      ...
      <div class="section-block">
        ...
        <div class="form-grid mt-2">
          ...
          <div>
            <b class="text-sm">Disahkan oleh (Ketua Program)</b>
...
function signaturePadHTML(role, reportId) {
  const label = role === 'penyelaras' ? 'Tandatangan Penyelaras Kursus' : 'Tandatangan Ketua Program';
  return `
...
function generateReportPDF(id) {
  ...
  const lecturers = safeParseArr(r.Pensyarah);
  fieldRow('Kelas & Pensyarah:', lecturers.join('; ') || '—');
  ...
  doc.text('Disahkan oleh (Ketua Program)', rightX, sigY);
  y = sigY + 5;
...
function renderPenggunaPage() {
  ...
  const rows = usersList.map(u => `
    <tr>
      <td>${esc(String(u.IC).replace(/(\d{6})(\d{6})/, '$1-$2'))}</td>
      <td>${esc(u.Nama)}</td>
  ...
  return `
    ...
    <div class="card">
      ...
      <div class="table-wrap">
        <table>
          <thead><tr><th>No. IC</th><th>Nama</th><th>Peranan</th><th>Tindakan</th></tr></thead>
          <tbody>${rows}</tbody>
        </table>
      </div>`}
    </div>`;
...
function roleLabel(role) {
  return role === 'admin' ? 'Pentadbir' : role === 'ketua' ? 'Ketua Program' : 'Penyelaras Kursus';
}
...
function openUserForm() {
  ...
  <div class="modal-bg open">
    <div class="modal modal-sm">
      ...
      <div class="form-group"><label>No. Kad Pengenalan</label><input id="u-ic" maxlength="12" placeholder="12 digit" oninput="this.value=this.value.replace(/\\D/g,'')"></div>
      <div class="form-group"><label>Nama Penuh</label><input id="u-nama"></div>
      <div class="form-group">
        <label>Peranan</label>
        <select id="u-role">
          <option value="penyelaras">Penyelaras Kursus</option>
          <option value="ketua">Ketua Program</option>
...
async function saveUserItem() {
  const ic = document.getElementById('u-ic').value.trim();
  const nama = document.getElementById('u-nama').value.trim();
  if (!/^\d{12}$/.test(ic)) { toast('No. IC mesti 12 digit angka.', 'error'); return; }
  ...
  return links.map(l => {
    ...
    <div class="section-block">
      <div class="flex justify-between items-center">
        ...
        <div>
          <button class="btn btn-outline btn-sm" onclick="editProgramLinkPLO('${l.ID}')">Edit PLO</button>
          <button class="btn btn-red btn-sm" onclick="removeProgramLink('${l.ID}','${esc(l.KodKursus)}')">Buang</button>
        </div>
      </div>
