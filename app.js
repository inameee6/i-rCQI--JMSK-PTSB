...
async function loadAllData() {
  try {
    const [reportsRes, laporanRes, courseRes, programRes, pkRes, pensyarahRes, kelasRes] = await Promise.all([
  ...
  <div class="modal-bg open" id="modal-report">
    <div class="modal">
      ...
      <div class="section-block">
        ...
        <div class="form-grid mt-2">
          <div class="form-group"><label>2.1 Kehadiran Mesyuarat</label>
            <div id="kehadiran-wrap" style="border:1px solid var(--border);border-radius:7px;padding:10px;background:#fff;min-height:44px;">
              ...
              <div id="kehadiran-empty" class="text-sm text-muted">Pilih kursus dahulu untuk papar senarai pensyarah.</div>
            </div>
          </div>
          ...
          <div class="table-wrap mt-1">
            <table style="font-size:11px;">
              ...
              <tbody><tr id="grade-row-inputs">${['A+','A','A-','B+','B','B-','C+','C','C-','D+','D','E','E-','F'].map(g => {
                ...
                return `<td><input type="number" step="0.1" data-grade="${g}" value="${esc(gd[g] || '')}" style="width:48px;padding:4px;font-size:11px;" oninput="autoCalcQO()"></td>`;
              }).join('')}</tr></tbody>
            </table>
          </div>
          <div class="form-hint mt-1">Jumlah tidak semestinya 100% — masukkan peratusan setiap gred.</div>
        </div>

        <div class="mt-2"><b class="text-sm">5.2 Quality Objectives (Auto-Kira)</b>
          <div class="section-block mt-1" style="background:var(--primary-light);">
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;">
              <div>
                <div class="text-sm text-muted mb-0">≥90% pelajar capai gred D ke atas</div>
                <div id="qo1-result" style="font-size:20px;font-weight:700;margin:4px 0;">—</div>
                <div id="qo1-calc" class="text-sm text-muted"></div>
              </div>
              <div>
                <div class="text-sm text-muted mb-0">≥25% pelajar capai gred B ke atas</div>
          ...
          <div class="form-grid mt-1">
            <div class="form-group">
              <label>Tindakan Pencegahan/Pembetulan (QO1)</label>
              <input id="f-qo1-tindakan" value="${esc(existing?.QualityObj1Tindakan)}" placeholder="Nyatakan tindakan jika perlu...">
            </div>
            <div class="form-group">
              <label>Tindakan Pencegahan/Pembetulan (QO2)</label>
              <input id="f-qo2-tindakan" value="${esc(existing?.QualityObj2Tindakan)}" placeholder="Nyatakan tindakan jika perlu...">
            </div>
          </div>
...
function refreshLecturerDatalist(kod) {
  ...
  document.querySelectorAll('#lecturer-rows .repeat-row').forEach(row => {
    ...
    if (pensyarahSel) {
      ...
      pensyarahSel.innerHTML = `<option value="">— Pilih Pensyarah —</option>` +
...
function autoCalcQO() {
  ...
  document.querySelectorAll('#grade-row-inputs input[data-grade]').forEach(inp => {
    grades[inp.dataset.grade] = parseFloat(inp.value) || 0;
  });
  ...
  // Update label threshold secara dinamik
  const lbl1 = document.querySelector('[for="f-qo1-capai"]');
  const lbl2 = document.querySelector('[for="f-qo2-capai"]');

  if (r1) {
    r1.textContent = allZero ? '—' : (qo1Pass ? '✅ Ya' : '❌ Tidak');
    r1.style.color = allZero ? 'var(--text-muted)' : (qo1Pass ? 'var(--success)' : 'var(--danger)');
    if (c1) c1.textContent = !allZero ? `Jumlah D ke atas: ${sumQO1.toFixed(1)}% (Threshold: ≥${threshold1}%)` : '';
  }
  if (r2) {
    r2.textContent = allZero ? '—' : (qo2Pass ? '✅ Ya' : '❌ Tidak');
    r2.style.color = allZero ? 'var(--text-muted)' : (qo2Pass ? 'var(--success)' : 'var(--danger)');
    if (c2) c2.textContent = !allZero ? `Jumlah B ke atas: ${sumQO2.toFixed(1)}% (Threshold: ≥${threshold2}%)` : '';
  }

  // Update label QO supaya tunjuk threshold sebenar
  const qo1Label = document.querySelector('#qo1-result')?.closest('.section-block')?.querySelector('.text-muted');
  document.querySelectorAll('#qo1-result, #qo2-result').forEach((el, i) => {
    const labelEl = el.previousElementSibling;
    if (labelEl && i === 0) labelEl.textContent = `≥${threshold1}% pelajar capai gred D ke atas`;
    if (labelEl && i === 1) labelEl.textContent = `≥${threshold2}% pelajar capai gred B ke atas`;
  });

  // Update hidden fields untuk simpan
  ...
  row.innerHTML = `
    ...
    <div>
      <input type="text" class="lec-kelas" value="${esc(kelas)}"
        placeholder="Taip atau pilih kelas" list="kelas-list-${uid}" autocomplete="off"
...
function generateReportPDF(id) {
  ...
  doc.text(`• ≥${qo1Th}% pelajar capai gred D ke atas: ` + (r.QualityObj1Capai || '—') + (r.QualityObj1Tindakan ? ' (' + r.QualityObj1Tindakan + ')' : ''), margin, y); y += 5;
  doc.text(`• ≥${qo2Th}% pelajar capai gred B ke atas: ` + (r.QualityObj2Capai || '—') + (r.QualityObj2Tindakan ? ' (' + r.QualityObj2Tindakan + ')' : ''), margin, y); y += 7;
      ...
      <div class="section-block mt-2" style="background:var(--primary-light);">
        ...
        <div class="form-grid mt-1">
          <div class="form-group mb-0">
            <label>QO1: % pelajar capai gred D ke atas (default: 90)</label>
            <input type="number" id="cm-qo1" value="${esc(qo1)}" min="0" max="100" step="1" placeholder="90">
          </div>
          <div class="form-group mb-0">
            <label>QO2: % pelajar capai gred B ke atas (default: 25)</label>
...
function renderPensyarahPage() {
  ...
  const cards = allKod.map(kod => {
    ...
    const pRows = (pGrouped[kod] || []).map(p => `
      <tr>
        ...
        <td><button class="btn btn-red btn-sm" onclick="deletePItem('${esc(p.ID)}')">Padam</button></td>
      </tr>`).join('');
    const kRows = (kGrouped[kod] || []).map(k => `
      <tr>
        <td>${esc(k.NamaKelas)}</td>
        <td><span class="tag tag-green">Kelas</span></td>
        <td><button class="btn btn-red btn-sm" onclick="deleteKItem('${esc(k.ID)}')">Padam</button></td>
      </tr>`).join('');
    return `
    <div class="card">
  ...
  return `
    ...
    <div class="btn-row">
      <button class="btn btn-blue" onclick="openPKFormNew('pensyarah')">＋ Tambah Pensyarah</button>
      <button class="btn btn-outline" onclick="openPKFormNew('kelas')">＋ Tambah Kelas</button>
    </div>
    ${allKod.length === 0 ? `<div class="card">${emptyState('👨‍🏫', 'Belum ada rekod. Klik "+ Tambah Pensyarah" atau "+ Tambah Kelas" untuk mula.')}</div>` : cards}`;
}

function openPKFormNew(jenis) {
  const courses = courseMasterList.map(c => `<option value="${esc(c.KodKursus)}">${esc(c.KodKursus)} — ${esc(c.NamaKursus)}</option>`).join('');
  const label = jenis === 'pensyarah' ? 'Nama Pensyarah' : 'Nama Kelas';
  ...
  <div class="modal-bg open">
    <div class="modal modal-sm">
      <div class="modal-title">${jenis === 'pensyarah' ? '👨‍🏫 Tambah Pensyarah' : '🏫 Tambah Kelas'}</div>
      <div class="form-group">
      ...
      <div class="modal-footer">
        <button class="btn btn-outline" onclick="closeDetailModal()">Batal</button>
        <button class="btn btn-blue" id="btn-save-pk2" onclick="savePKNew('${jenis}')">Simpan</button>
      </div>
    </div>
  </div>`;
}
...
async function deletePItem(id) {
  if (!confirm('Padam pensyarah ini?')) return;
  try {
